...
PART = "2020_y_pulley_mount_assembly";

/***************************************************************/

if (PART == "2020_y_pulley_mount_assembly") {
  brace_offset = 0;
  brace_length = 26;
  pully_x = -18;
  pully_y = 18;
  ...
  rotate([0,0,90]) {
    ...
    // pully Slide
    translate([pully_x, 15 ,pully_height+4]) 2020_y_pulley_slide();
...
if (PART == "2020_y_pulley_mount") {
  color(part_color)
  2020_y_pulley_mount();
}

if (PART == "2020_y_pulley_slide") {
  color(part_color)
  2020_y_pulley_slide();
}
...
module 2020_y_pulley_mount(
  ...
  difference(){
    ...
    union(){
      ...
      // Slide run back
      translate([slide_x+2, -20, 8]) cube([32, 6, 24], center=true);
...
/***************************************************************/
 ...
 module 2020_y_pulley_slide() {
  difference(){
    hull() {
      translate([0,-3, 0]) cube([24, 20, 19], center=true);
      translate([0,-11, 0]) sphere(d=19, center=true, $fn=100);
      translate([0, 6, 0]) cylinder(h=19, d=24, $fn=100, center=true);
    }
    hull() {
      translate([0, 8, 0]) cube([20, 40, 11], center=true);
      translate([0,-14, 0]) sphere(d=10, center=true, $fn=100);
    }
    translate([0, 6, 0]) cylinder(h=20, d=M5_bolt_hole, $fn=100, center=true);
